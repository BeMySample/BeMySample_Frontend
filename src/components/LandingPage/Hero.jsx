...
const Hero = ({ language }) => {
	return (
		<div className="flex flex-col items-center justify-center w-full bg-white pt-[120px] px-4 sm:px-6 lg:px-8">
			<motion.div
				className="flex flex-col items-center w-full gap-4 sm:gap-6 lg:gap-8"
				initial={{ opacity: 0, y: 50 }}
				animate={{ opacity: 1, y: 0 }}
				transition={{ duration: 0.8 }}
			>
				<motion.p
					className="text-[28px] sm:text-[36px] md:text-[45px] lg:text-[50px] text-center font-bold"
					initial={{ opacity: 0 }}
					animate={{ opacity: 1 }}
			...
			<motion.img
				className="mt-8 sm:mt-10 w-full max-w-xs sm:max-w-md md:max-w-lg lg:max-w-xl rounded-xl"
