...
import SurveyEdit from "./pages/Dashboard/SurveyEdit";
        ...
        <NavBar
          ...
          childrenCenter={
            <div className="absolute inset-0 top-0 left-0 right-0 w-full -z-10 flex-grow flex items-center justify-center space-x-8 text-base">
              {["Sunting", "Bagikan", "Hasil"].map((menu) => (
                <button
                  key={menu}
                  onClick={() => handleMenuClick(menu)}
                  className={`relative font-normal focus:outline-none ${
                    activeMenu === menu ? "text-blue-600" : "text-gray-600"
                  }`}
                >
                  {menu}
                  {activeMenu === menu && (
                    <span className="absolute -top-8 left-0 right-0 h-[8px] bg-gradient-to-r from-[#1F38DB] to-[#30ADD7] rounded-b-md"></span>
                  )}
                </button>
              ))}
            </div>
